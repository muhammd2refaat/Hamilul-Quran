...
const S = {
  en: {
    navPrograms: 'Programs', navMethod: 'Methodology', navTrial: 'Free trial', navFaq: 'FAQ',
  ...
  ar: {
    navPrograms: 'البرامج', navMethod: 'منهجيتنا', navTrial: 'حصة تجريبية', navFaq: 'الأسئلة الشائعة',
...
export function LandingPage({ initialLang = 'en' }: { initialLang?: Lang }) {
  ...
  return (
    <div dir={dir} style={{ fontFamily: "'Manrope','Tajawal',sans-serif", color: '#10241C', background: '#F1EBDD', overflowX: 'hidden' }}>
      ...
      <header style={{ background: DARK, position: 'relative', zIndex: 10 }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', padding: '22px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 }}>
          ...
          <nav className="ee-nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: 20, fontSize: 14, fontWeight: 500, color: '#B9CBBC' }}>
            ...
            <a href="#faq" style={{ color: 'inherit', textDecoration: 'none' }}>{t.navFaq}</a>
            <a href="#trial" style={{ color: 'inherit', textDecoration: 'none' }}>{t.navTrial}</a>
            <a
              href={lang === 'ar' ? '/' : '/ar'}
              onClick={(e) => { e.preventDefault(); setLang(lang === 'ar' ? 'en' : 'ar'); }}
              className="ee-lang"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'transparent', border: '1px solid rgba(185,203,188,.4)', color: '#F1EBDD', padding: '8px 14px', borderRadius: 30, fontSize: 13, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer', transition: 'all .25s', textDecoration: 'none' }}
            >
        ...
        {mobileNavOpen && (
          ...
          >
            ...
            <a href="#faq" onClick={() => setMobileNavOpen(false)} style={{ color: 'inherit', textDecoration: 'none', padding: '10px 0' }}>{t.navFaq}</a>
            <a href="#trial" onClick={() => setMobileNavOpen(false)} style={{ color: 'inherit', textDecoration: 'none', padding: '10px 0' }}>{t.navTrial}</a>
            <a
              href={lang === 'ar' ? '/' : '/ar'}
              onClick={(e) => { e.preventDefault(); setMobileNavOpen(false); setLang(lang === 'ar' ? 'en' : 'ar'); }}
              className="ee-lang"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'transparent', border: '1px solid rgba(185,203,188,.4)', color: '#F1EBDD', padding: '10px 14px', borderRadius: 30, fontSize: 13.5, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer', transition: 'all .25s', margin: '10px 0', width: 'fit-content', textDecoration: 'none' }}
            >
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#D9B45F', display: 'inline-block' }} />
              {t.langBtnLabel}
            </a>
      ...
      {/* ── FAQ ── */}
      <section id="faq" style={{ background: '#F1EBDD', padding: '96px 36px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ fontFamily: "'Reem Kufi',sans-serif", color: '#B08A2E', fontSize: 20, marginBottom: 6 }}>{t.faqEyebrow}</div>
          <h2 style={{ fontFamily: "'Space Grotesk','Reem Kufi',sans-serif", fontSize: 38, fontWeight: 600, color: '#10241C', letterSpacing: -.5, lineHeight: 1.15, marginBottom: 36 }}>{t.faqTitle}</h2>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {faqs.map((f, i) => {
              const open = openFaq === i;
              return (
                <div key={i} style={{ background: '#fff', border: '1px solid rgba(12,51,38,.12)', borderRadius: 12, overflow: 'hidden' }}>
                  <button
                    onClick={() => setOpenFaq(open ? null : i)}
                    aria-expanded={open}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, textAlign: 'start', background: 'transparent', border: 'none', cursor: 'pointer', padding: '18px 20px', fontFamily: 'inherit' }}
                  >
                    <span style={{ fontSize: 15.5, fontWeight: 600, color: '#10241C' }}>{f.q}</span>
                    <ChevronDown size={18} color="#5C6B5F" style={{ flexShrink: 0, transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }} />
                  </button>
                  {open && (
                    <p style={{ margin: 0, padding: '0 20px 20px', fontSize: 14, lineHeight: 1.7, color: '#5C6B5F' }}>{f.a}</p>
                  )}
                </div>
              );
            })}
