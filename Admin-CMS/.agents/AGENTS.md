...
* **Styling:** Tailwind CSS with plugins (`@tailwindcss/forms`, `@tailwindcss/typography`). Icons via `lucide-react`.
* **State & Data Fetching:** `zustand` for global client state, `@tanstack/react-query` for server state and API interactions, and `axios` for HTTP requests.
* **Routing:** `react-router-dom` (v6).
* **Forms & Validation:** `react-hook-form` paired with `@hookform/resolvers` and `zod` for schema validation.
* **UI Components:** Utilizes `@tanstack/react-table` for tables, `@tanstack/react-virtual` for large lists, `recharts` for charts, and `@tiptap/react` for rich text editing.
* **Development Flow:** Use `npm run dev` or `yarn dev` to start the Vite development server.
...
* **Terminal:** Use `ctx_shell` for testing, building, or ecosystem commands (`npm run dev`, `npm run lint`, `npm run build`).
